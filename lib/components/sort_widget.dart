...
class _SortWidgetState extends State<SortWidget> {
  ...
  void initState() {
    currentAction =
        widget.model ?? SortParam(name: LocaleKeys.defaultArrangement);
    super.initState();
  }
        ...
        Container(
          ...
          child: Column(
            children: [
              ...
              itemSort(
                LocaleKeys.defaultArrangement,
                SortParam(name: LocaleKeys.defaultArrangement),
                (action) => widget.callBack.call(action),
              ),
              itemSort(
                LocaleKeys.theRouteName,
                SortParam(name: LocaleKeys.theRouteName, orderType: 1),
                (action) => widget.callBack.call(action),
              ),
              itemSort(
                LocaleKeys.authorDes,
                SortParam(
                    name: LocaleKeys.authorDes, orderValue: 0, orderType: 2),
                (action) => widget.callBack.call(action),
              ),
              itemSort(
                LocaleKeys.authorAsc,
                SortParam(
                    name: LocaleKeys.authorAsc, orderValue: 1, orderType: 2),
                (action) => widget.callBack.call(action),
              ),
              itemSort(
                LocaleKeys.userDes,
                SortParam(
                    name: LocaleKeys.userDes, orderType: 3, orderValue: 0),
                (action) => widget.callBack.call(action),
              ),
              itemSort(
                LocaleKeys.userAsc,
                SortParam(
                    name: LocaleKeys.userAsc, orderType: 3, orderValue: 1),
                (action) => widget.callBack.call(action),
              ),
            ],
          ),
