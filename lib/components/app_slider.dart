...
class AppSlider extends StatefulWidget {
  ...
  final AppSliderHandler? handler;
  final AppSliderHandler? rightHandler;
  ...
  final List<AppSliderFixedValue>? fixedValues;
  final bool rangeSlider;
  final bool rtl;
  final bool jump;
  final bool selectByTap;
  final List<AppSliderIgnoreSteps> ignoreSteps;
  final bool disabled;
  final double? touchSize;
  final bool visibleTouchArea;
  final double minimumDistance;
  final double maximumDistance;
  final AppSliderHandlerAnimation handlerAnimation;
  final AppSliderTooltip? tooltip;
  final AppSliderTrackBar trackBar;
  final AppSliderStep step;
  final AppSliderHatchMark? hatchMark;
  ...
  AppSlider(
      ...
      this.tooltip,
      this.trackBar = const AppSliderTrackBar(),
      this.handlerAnimation = const AppSliderHandlerAnimation(),
      this.selectByTap = true,
      this.step = const AppSliderStep(),
      this.hatchMark,
...
class _AppSliderState extends State<AppSlider> with TickerProviderStateMixin {
  ...
  late AppSliderTooltip _tooltipData;
  ...
  double? _widgetMin;
  double? _widgetMax;
  List<AppSliderIgnoreSteps> _ignoreSteps = [];
  List<AppSliderFixedValue> _fixedValues = [];
...
//                  ..._points,
  ...
  void initMethod() {
    ...
    switch (_tooltipData.direction) {
      case AppSliderTooltipDirection.top:
        animationFinish = Offset(0, -1);
        break;
      case AppSliderTooltipDirection.left:
        animationFinish = Offset(-1, 0);
        break;
      case AppSliderTooltipDirection.right:
        animationFinish = Offset(1, 0);
        break;
      default:
  ...
  void _drawHatchMark() {
    ...
    AppSliderHatchMark hatchMark = AppSliderHatchMark();
    ...
    hatchMark.bigLine = widget.hatchMark!.bigLine ??
        AppSliderSizedBox(
            ...
            decoration: BoxDecoration(color: Colors.black45));
    hatchMark.labelBox =
        widget.hatchMark!.labelBox ?? AppSliderSizedBox(height: 50, width: 50);
...
//      left = hatchMark.linesDistanceFromTrackBar - 3.62;
      ...
      if (widget.axis == Axis.horizontal) {
        if (hatchMark.linesAlignment == AppSliderHatchMarkAlignment.left) {
      ...
      for (int p = 0; p <= percent; p++) {
        AppSliderSizedBox? barLineBox = hatchMark.smallLine;
...
//        left = (p * distance) + _handlersPadding - labelBoxHalfSize - 0.5;
          ...
          if (hatchMark.linesAlignment == AppSliderHatchMarkAlignment.right) {
    ...
    if (hatchMark.labels != null && hatchMark.labels!.length > 0) {
      ...
      double? tr;
      for (AppSliderHatchMarkLabel markLabel in hatchMark.labels!) {
...
//          left = (tr * distance) + _handlersPadding - labelBoxHalfSize - 0.5;
  ...
  void _setParameters() {
    ...
    if (widget.fixedValues != null && widget.fixedValues!.length > 0) {
      ...
      for (AppSliderFixedValue fixedValue in widget.fixedValues!) {
      ...
      for (double fixedPercent = 0; fixedPercent <= 100; fixedPercent++) {
        ...
        for (AppSliderFixedValue fixedValue in widget.fixedValues!) {
          if (fixedValue.percent == fixedPercent.toInt()) {
            ...
            if (fixedPercent > lowerIgnoreBound + 1 || lowerIgnoreBound == 0) {
              ...
              _ignoreSteps.add(AppSliderIgnoreSteps(
        ...
        _fixedValues.add(
            AppSliderFixedValue(percent: fixedPercent.toInt(), value: fValue));
      ...
      if (!fixedV.contains(100)) {
        _ignoreSteps
            .add(AppSliderIgnoreSteps(from: biggestPoint! + 1, to: 101));
      }
    } else {
    ...
    AppSliderTooltip widgetTooltip = widget.tooltip ?? AppSliderTooltip();

    _tooltipData = AppSliderTooltip();
    ...
    _tooltipData.direction =
        widgetTooltip.direction ?? AppSliderTooltipDirection.top;
  ...
  void _generateHandler() {
    /*Right Handler Data*/
    AppSliderHandler inputRightHandler =
        widget.rightHandler ?? AppSliderHandler();
  ...
  bool _leftHandlerIgnoreSteps(double? tS) {
    ...
    if (_ignoreSteps.length > 0) {
      ...
      for (AppSliderIgnoreSteps steps in _ignoreSteps) {
        if (((!widget.rtl) &&
  ...
  void _checkRangeStep(double realValue) {
    ...
    if (widget.step.rangeList != null) {
      for (AppSliderRangeStep rangeStep in widget.step.rangeList!) {
...
//    items      ..addAll(_points);
  ...
  Positioned _tooltip(
    ...
    if (_tooltipData.direction == AppSliderTooltipDirection.top) {
...
class _MakeHandler extends StatelessWidget {
  ...
  final double? height;
  final GlobalKey? id;
  final AppSliderHandler? handlerData;
  final bool? visibleTouchArea;
  ...
  Widget build(BuildContext context) {
    ...
    AppSliderHandler handler = handlerData ?? AppSliderHandler();
    handler.child ??= iconWidget();
    if (handlerIndex == 2) {
      handler.child ??=
              iconWidget() /*Icon(
    ...
    handler.decoration ??= BoxDecoration(boxShadow: [
      BoxShadow(
          ...
          spreadRadius: 0.2,
          offset: Offset(0, 1))
    ], color: colorOrange100, shape: BoxShape.circle);
...
class AppSliderTooltip {
  ...
  AppSliderTooltipDirection? direction;
  AppSliderTooltipPositionOffset? positionOffset;
...
class AppSliderTooltipPositionOffset {
  double? top;
  double? left;
  double? right;
  double? bottom;

  AppSliderTooltipPositionOffset(
...
class AppSliderTooltipBox {
  final BoxDecoration? decoration;
  final BoxDecoration? foregroundDecoration;
  final Matrix4? transform;

  const AppSliderTooltipBox(
...
class AppSliderIgnoreSteps {
  final double? from;
  final double? to;

  AppSliderIgnoreSteps({this.from, this.to})
...
class AppSliderFixedValue {
  final int? percent;
  final dynamic value;

  AppSliderFixedValue({this.percent, this.value})
      : assert(
...
class AppSliderHandlerAnimation {
  final Curve curve;
  final Curve? reverseCurve;
  final Duration duration;
  final double scale;

  const AppSliderHandlerAnimation(
...
class AppSliderHatchMark {
  bool disabled;
  double density;
  double? linesDistanceFromTrackBar;
  double? labelsDistanceFromTrackBar;
  List<AppSliderHatchMarkLabel>? labels;
  AppSliderSizedBox? smallLine;
  AppSliderSizedBox? bigLine;
  AppSliderSizedBox? labelBox;
  AppSliderHatchMarkAlignment linesAlignment;
  bool? displayLines;
  ...
  AppSliderHatchMark(
      ...
      this.smallLine,
      this.bigLine,
      this.linesAlignment = AppSliderHatchMarkAlignment.right,
      this.labelBox,
      this.displayLines})
...
class AppSliderHatchMarkLabel {
  final double? percent;
  final Widget? label;

  AppSliderHatchMarkLabel({
    this.percent,
    this.label,
...
class AppSliderStep {
  ...
  final List<AppSliderRangeStep>? rangeList;
...
enum AppSliderTooltipDirection { top, left, right }

enum AppSliderHatchMarkAlignment { left, right }
